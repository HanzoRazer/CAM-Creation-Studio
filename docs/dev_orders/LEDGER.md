...
merged as #21. Every other branch in this index is merged, closed, or
...
3. ~~**Freeze importer feature expansion.**~~ **In force.** The DXF importer is
   ...
   orders.
4. **Authorize the first neutral-geometry consumer.** ← **now live.** The
   importer produces evidence nothing consumes; that is the next real product
   question. It needs its own dev order and is deliberately unnumbered here —
   closure does not get to assign the next capability. Do not start another
   importer cleanup by default.
...
The next product-level question is the first authorized workspace that consumes
trustworthy `GeometryCollection` data without yet generating toolpaths or
G-code.
